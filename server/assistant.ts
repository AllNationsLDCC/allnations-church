...
- Collect: Name, Phone, Email, Type of Appointment (Pastoral Counseling, Ministry Inquiry, Prayer Request, General Meeting), Preferred Date/Time, Brief Message
...
router.post("/chat", async (req: Request, res: Response) => {
  try {
    ...
    const reply = completion.choices[0]?.message?.content || "I'm sorry, I couldn't process that. Please call us at (772) 343-7021.";
    return res.json({ reply });
  } catch (error) {
    console.error("[Assistant Chat Error]", error);
    return res.status(500).json({
      reply: "I'm having trouble connecting right now. Please call us at (772) 343-7021 or email AllNationsldcc@gmail.com and we'll be happy to help!",
    });
  }
});
...
Please respond to the visitor at: ${email}
    ...
    // Try to send via SMTP if configured, otherwise log and return success
    ...
    if (smtpUser && smtpPass) {
      ...
      // Log to console if SMTP not configured — still returns success to user
      console.log("[APPOINTMENT REQUEST — SMTP not configured, logging only]");
      console.log(emailBody);
    }

    return res.json({ success: true, message: "Your appointment request has been received! We will contact you within 1-2 business days." });
  } catch (error) {
    console.error("[Appointment Email Error]", error);
    return res.status(500).json({ error: "Failed to send appointment request. Please call (772) 343-7021 directly." });
  }
});
